...
- `artifacts/mobile/` — Expo mobile app
  ...
  - `app/(tabs)/wardrobe.tsx` — Wardrobe with working category filters
  - `app/(tabs)/shop.tsx` — Shop with product detail modal
  - `app/(tabs)/wishlist.tsx` — Wishlist
  - `app/(tabs)/stylist.tsx` — AI Stylist Chat tab (Aura)
...
- `artifacts/api-server/src/routes/chat.ts` — POST /api/chat (GPT-4.1-mini, profile-aware)
...
- Token-based caller identity: mobile must GET /api/auth/token (HMAC-signed, 10 min TTL) before calling /api/analyze — prevents unauthenticated proxy abuse
- Chat does NOT require the analyze token — uses its own rate limiter (60 req/15min per IP)
- Per-IP rate limiting (10 req/15 min analyze, 60 req/15 min chat) + global concurrency cap protect OpenAI quota
- MIME allowlist + base64 size cap (6 MB) applied before any OpenAI call
- 5-tab bottom nav: Home, Wardrobe, Wishlist, Shop, Stylist (chat)
- Wardrobe filters use keyword-matching on fashion recommendation text (Casual/Formal/Evening/Weekend)
- Shop product taps open a bottom-sheet modal with full description, match reason, and CTA
...
- Daily rotating tip card on home screen (tappable → opens AI Stylist Chat)
- Season badge pill on profile card; "Details" button links to profile screen
- Browse curated mock product recommendations in the Shop tab (tap for detail modal)
- Filter fashion recommendations by style category (Casual/Formal/Evening/Weekend) in Wardrobe
- **AI Stylist Chat (Aura)** — conversational AI tab that knows the user's full profile; suggests starter questions; supports multi-turn conversation; rate-limited
...
- Chat token (for /api/analyze) is cached for 8 min client-side (token TTL is 10 min) to avoid fetching a new token on every analysis
