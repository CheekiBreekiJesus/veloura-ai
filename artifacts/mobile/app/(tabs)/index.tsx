...
function buildCategories(analysis: AnalysisResult): CategoryCard[] {
  return [
    {
      title: "Skincare",
      subtitle: analysis.beauty_recommendations[0] ?? "Routine for you",
      icon: "water-outline",
      gradient: ["#FDECD3", "#F5D5B0"],
    },
    {
      title: "Cosmetics",
      subtitle: analysis.beauty_recommendations[1] ?? "Your glam look",
      icon: "sparkles-outline",
      gradient: ["#F0E4F5", "#DFC8EF"],
    },
    {
      title: "Hairstyle",
      subtitle: analysis.hairstyle_suggestions[0] ?? "Flatters your shape",
      icon: "cut-outline",
      gradient: ["#D9EEF5", "#B8DCEA"],
    },
    {
      title: "Outfit",
      subtitle: analysis.fashion_recommendations[0] ?? "Curated for you",
      icon: "shirt-outline",
...
type Recommendation = { title: string; desc: string; icon: React.ComponentProps<typeof Ionicons>["name"] };

function buildRecommendations(analysis: AnalysisResult): Recommendation[] {
  const recs: Recommendation[] = [];
  analysis.beauty_recommendations.forEach((r, i) => {
    if (i === 0) recs.push({ title: "Glow Routine", desc: r, icon: "sunny-outline" });
    if (i === 1) recs.push({ title: "Makeup Look", desc: r, icon: "color-palette-outline" });
  });
  if (analysis.hairstyle_suggestions[0]) {
    recs.push({ title: "Hairstyle Tip", desc: analysis.hairstyle_suggestions[0], icon: "cut-outline" });
  }
  if (analysis.fashion_recommendations[0]) {
    recs.push({ title: "Style Direction", desc: analysis.fashion_recommendations[0], icon: "shirt-outline" });
  }
  if (analysis.glasses_suggestions[0]) {
    recs.push({ title: "Eyewear Pick", desc: analysis.glasses_suggestions[0], icon: "glasses-outline" });
  }
  return recs;
}
    ...
    >
            ...
            <View style={styles.profileTraits}>
              {[
                ...
                <View key={i} style={styles.traitRow}>
                  ...
                  />
                  <Text
                    style={[styles.traitText, { color: colors.foreground }]}
                  >
                    {t.label}
                  </Text>
                </View>
      ...
      <View style={styles.sectionPad}>
        <View style={styles.paletteRow}>
          {analysis.color_palette.slice(0, 6).map((hex, i) => (
    ...
    >
      ...
      >
        ...
        <View style={styles.previewTags}>
          {["Warm Undertone", "Oval Face", "Soft Classic", "Wavy Hair"].map(
            (tag, i) => (
              <View
                key={i}
                style={[
...
const styles = StyleSheet.create({
  ...
  greeting: { fontSize: 28, fontFamily: "Inter_700Bold", marginBottom: 4 },
  ...
  onboardLogo: { alignItems: "center", marginBottom: 32 },
  logoGrad: {
    width: 72,
    height: 72,
    borderRadius: 22,
