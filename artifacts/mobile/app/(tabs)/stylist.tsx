...
function buildWelcome(analysis: ReturnType<typeof useAnalysis>["analysis"]): string {
  if (!analysis) {
    return "Hi! I'm your AI stylist. Upload a selfie first to get fully personalized advice, or ask me any beauty and fashion question!";
  }
  return `Hi! I'm your AI stylist. I've reviewed your profile — you're a ${analysis.style_archetype} with ${analysis.undertone} undertones and a ${analysis.face_shape} face shape. What would you like to know?`;
}
  ...
  return (
    <View style={[styles.bubbleRow, isUser ? styles.bubbleRowUser : styles.bubbleRowAssistant]}>
      {!isUser && (
        ...
        >
          <Ionicons name="sparkles" size={14} color="#C4956A" />
        </LinearGradient>
      )}
      <View
        style={[
...
function TypingIndicator({ colors }: { colors: ReturnType<typeof useColors> }) {
  return (
    <View style={[styles.bubbleRow, styles.bubbleRowAssistant]}>
      <LinearGradient colors={["#F5EDE3", "#E8C4A0"]} style={styles.avatarWrap}>
        <Ionicons name="sparkles" size={14} color="#C4956A" />
      </LinearGradient>
      <View
        style={[
          styles.bubble,
          { backgroundColor: colors.card, borderColor: colors.border, borderWidth: 1, minWidth: 60 },
        ]}
      >
        ...
        <View style={styles.headerInner}>
          ...
          <View>
            ...
            <Text style={[styles.headerSub, { color: colors.mutedForeground }]}>
              {analysis ? `Personalized for your profile` : "Ask me anything"}
            </Text>
          </View>
        </View>
        {!analysis && (
...
const styles = StyleSheet.create({
  ...
  messages: { flex: 1 },
  messagesContent: { paddingHorizontal: 16, paddingTop: 8, gap: 10 },
  quickPrompts: { marginBottom: 12, gap: 8 },
  quickLabel: { fontSize: 12, fontFamily: "Inter_500Medium", marginBottom: 2 },
  quickChip: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
